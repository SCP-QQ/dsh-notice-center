	...
	try {
		...
		*/
		function WhaleSettingsSection({ scope, t }) {
			...
			const notifyForeground = value.notifyForeground ?? NOTIFY_DEFAULTS.notifyForeground;
			...
			/** 改音量后试听当前设置的完成提示音：250ms 防抖，只在停手后按新音量放一次。
			*  primeChime 必须在 onChange 这个用户手势里调，否则防抖后 resume 可能被自动播放策略拦。 */
			const previewVolume = (next) => {
				scope.set("notifyVolume", next);
				primeChime();
				if (volumeTimer.current !== void 0) clearTimeout(volumeTimer.current);
			...
			*  箭头仍是带 aria-expanded 的真按钮，键盘可达；整行点击只是给鼠标加热区。 */
			const sectionRow = (key, title, hint, checked, onChange, expanded, onToggle, expandLabel) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(primitives.Switch, {
							checked,
							label: title,
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					notifyExpanded ? switchRow("notifyForeground", t("notifyForeground"), t("notifyForegroundHint"), notifyForeground, (next) => scope.set("notifyForeground", next)) : null,
...
//#endregion
		...
		*/
		...
		function apply(ctx) {
			...
			* @param interaction - pendingInteractions 的 entry；缺席时为 undefined。
			...
			*/
			...
			function detectTransitions(state) {
				...
				if (pendingSeen) for (const id of current) {
					...
					queueNotification("pending", id, label, pendingTypeLabel(pendingMap?.get(id)));
				}
			...
			*/
			...
			/** running true→false 边沿跟踪（只跟踪主会话）：完成时恰好被选中 →
			*  绿灯记入 finishedWhileHidden（仅当标签页不在台前；台前完成不记，V0-02）；
			*  通知则无条件入队，由 flush 按「前台也提醒」统一裁决。官方 syncCompletedNotifications
			*  只报"未选中时完成"（`sessionId !== selected` 才 arm），选中的空缺在这里补齐。
			*  重新运行、会话移除均清除。 */
			function trackEdges(state) {
				for (const row of Object.values(state.byId)) {
					...
					if (prev && !row.running) {
						...
						/* 通知另算：选中会话完成时官方不置 completed，这条是它的唯一来源，
						   因此「前台要不要打扰」交给 flush 按 notifyForeground 统一裁决。 */
			...
			function targetOf(state) {
				...
				for (const row of Object.values(state.byId)) {
					if (row.origin === "subagent") continue;
					if (pendingMap?.has(row.id) || row.pendingInteraction !== void 0) return uri(c.amber);
